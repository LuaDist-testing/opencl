...
table.insert(tests, function(device)
  ...
  local N = 2 ^ 10
  local d_buf = context:create_buffer(N * ffi.sizeof("cl_uint"))
  kernel:set_arg(0, 32 * ffi.sizeof("cl_uint"))
  kernel:set_arg(1, d_buf)
  queue:enqueue_ndrange_kernel(kernel, nil, {N}, {32})
  local buf = ffi.cast("cl_uint *", queue:enqueue_map_buffer(d_buf, true, "read", 0, N * ffi.sizeof("cl_uint")))
  for i = 0, N - 1 do
    assert(buf[i] == i + 1)
  end
...
table.insert(tests, function(device)
  ...
  assert(kernel:get_work_group_info(device, "work_group_size") == device:get_info("max_work_group_size"))
